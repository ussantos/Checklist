...
Na tela **Gestão Pedagógica > Agenda**, o botão **Sincronizar Sponte** busca a agenda de cada aluno ativo importado do Sponte pelo endpoint SOAP `GetAgendaAluno` e aproveita somente a seção **AulasLivres**. Quando o SOAP trouxer a **Situação da Aula**, o Checklist usa as mesmas nomenclaturas do Sponte: `Presença`, `Falta`, `Não dada` e `Cancelada`. Para auditorias e correções em lote, exporte o relatório Sponte **Aulas Livres** em XML com Situação da Aula = Todas e use o bloco **Corrigir situações pelo XML do Sponte** na **Agenda de Aulas** do administrador ou na **Agenda do Instrutor**. Também é possível rodar primeiro `reconcile_sponte_lessons --report-xml caminho.xml --audit-only`; para aplicar a correção pelo terminal, rode sem `--audit-only`. A informação do relatório prevalece sobre o banco local. Essas aulas regulares são exibidas no Checklist como somente leitura. O Checklist continua criando localmente apenas **Aulas Experimentais ou Play**. Sincronizações iniciadas pelas telas rodam em segundo plano; o usuário recebe uma notificação ao iniciar e outra ao concluir ou falhar.
...
O endpoint SOAP `GetSalas` deve ser chamado com todos os filtros esperados pelo WSDL, mesmo vazios: `nSalaID`, `sSigla`, `sDescricao`, `nAtivo` e `sParametrosBusca`.
...
The SOAP `GetSalas` endpoint must be called with every WSDL filter, even when empty: `nSalaID`, `sSigla`, `sDescricao`, `nAtivo`, and `sParametrosBusca`.
